...
function getRootPath(){
    return ltrim($_SESSION['directory_path'],'/');
}


function Authenticate(){
    $db = new DB_Class();
    $query = "SELECT * FROM users WHERE user_name = '" . $_POST['fm_usr'] . "' AND password = '" . md5($_POST['fm_pwd']) . "'";
    $sql = $db->db_connect();
    $db->db_query($query);
    $user = $db->db_row();
    $db->db_free_result();
    $_SESSION['directory_path'] = $root_path.'/'.$user[5];
...
function getUserType(){
    if(isset($_SESSION) && !empty($_SESSION['userdata'])){
        return $_SESSION['userdata']['user_type'];
    }
}
